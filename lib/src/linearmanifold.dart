...
class LinearManifold<T extends num?> extends MembershipFunction<T> {
  ...
  LinearManifold(List<List<num?>> inputList)
    ...
    for (var i = 0; i < inputList.length - 1; i++) {
      var from = _LinearManifoldPoint(inputList[i][0], inputList[i][1]);
      var to = _LinearManifoldPoint(inputList[i + 1][0], inputList[i + 1][1]);
      segments.add(_LinearManifoldSegment(from, to));
    }
  }
